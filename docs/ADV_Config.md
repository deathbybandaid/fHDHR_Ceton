...
* `tuner_count` is a limit of devices able to stream from the script. The default is 4, some tuners support 6
* `friendlyname` is to set the name that Plex sees the script as.
* `stream_type` can be set to `ffmpeg`, `vlc` 
...
* `method` defaults to `origin` and will not pull any epg data. Other Options include `blocks` which is an hourly schedule with minimal channel information. Another option is `zap2it`, which is another source of EPG information. Channel Numbers may need to be manually mapped.
